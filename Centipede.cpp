...
void movecentipede(float centipede[][4], bool& shift, float mushroom[][4], float cvelocity, int bodyparts, const int mushrooms);

//Mushroom Related
void initializeMushrooms(float mushroom[][4], sf::Texture& mushroomTexture, sf::Sprite mushroomSprite[], const int mushrooms, int mushroomhealth[], int m);
void drawmushroom(sf::RenderWindow& window, float mushroom[][4], sf::Sprite mushroomSprite[], const int mushrooms);
...
void collision(sf::RenderWindow& window, float centipede[][4], float bullets[][3], sf::Sprite centipedeSprite[], sf::Texture& centipedeheadTexture, float cvelocity, const int maxbodyparts,
	const int maxBullets, float mushroom[][4], const int mushrooms, int& m, int& points, sf::Sprite mushroomSprite[]);
void playerCollision(float player[], float centipede[][4], int bodyparts, float mushroom[][4], int mushroomhealth[], int points);
void mushroomcollision(sf::RenderWindow& window, float mushroom[][4], float bullets[][3], const int maxBullets, const int mushrooms, int mushroomhealth[], int& points, sf::Sprite mushroomSprite[]);
void playermushroomcollision(sf::RenderWindow& window, int points, float player[], float mushroom[][4], int mushrooms);
...
void spidermushroomcollision(float spider[], float mushroom[][4], int mushrooms, sf::Sprite mushroomSprite[]);
...
void movescorpion(sf::RenderWindow& window, float scorpion[], float scorpionvelocity, float mushroom[][4], const int mushrooms, sf::Sprite mushroomSprite[], sf::Texture& poisonmushroomTexture);
...
int main()
{
    ...
    if (!playerTexture.loadFromFile("Textures/spaceship.png")) {
	...
	// Initializing Mushroom and Mushroom Sprites. 4th column is poison flag (0 = safe, 1 = poisoned)
	float mushroom[mushrooms][4] = {};
	...
	//delete playerTexture;  // Clean up texture
}
...
void initializeMushrooms(float mushroom[][4], sf::Texture& mushroomTexture, sf::Sprite mushroomSprite[], const int mushrooms, int mushroomhealth[], int m) {
	...
	for (int i = 0; i < mushrooms; i++) {
		...
		// poison flag: 0 = not poisoned, 1 = poisoned by scorpion
		mushroom[i][3] = 0;
	}
	...
	for (int i = 0; i < m; i++) {
		...
		mushroom[i][exists] = 1;
		// ensure poison flag is clear for newly spawned mushrooms
		mushroom[i][3] = 0;
	}
}
void initializeCentipede(float centipede[][4], sf::Texture& centipedeTexture, sf::Texture& centipedeheadTexture, sf::Sprite centipedeSprite[], int bodyparts, int maxbodyparts) {
...
/////////////////////////////////////////////////////////////////////////////
void drawPlayer(sf::RenderWindow& window, float player[], sf::Sprite playerSprite) {
    if (player != nullptr) {  // Add safety check
...
void movescorpion(sf::RenderWindow& window, float scorpion[], float scorpionvelocity, float mushroom[][4], const int mushrooms, sf::Sprite mushroomSprite[]
	, sf::Texture& poisonmushroomTexture) {
	if (scorpion[3] == 1) {
	...
	if (scorpion[exists] == true) {
		for (int k = 0; k < mushrooms; k++) {
			...
			if ((scorpion[x] <= mushroom[k][x] + boxPixelsX) &&
				...
				// mark this mushroom as poisoned by scorpion
				mushroom[k][3] = 1;
...
void playermushroomcollision(sf::RenderWindow& window, int points, float player[], float mushroom[][4], int mushrooms) {
	for (int i = 0; i < mushrooms; i++) {
		// only lethal if mushroom exists and is poisoned by scorpion (poison flag == 1)
		if (mushroom[i][exists] == 1 && mushroom[i][3] == 1) {
...
void spidermushroomcollision(float spider[], float mushroom[][4], int mushrooms, sf::Sprite mushroomSprite[]) {
	for (int i = 0; i < mushrooms; i++) {
		if ((spider[x] <= mushroom[i][x] + boxPixelsX) &&
			...
			// clear poison flag as mushroom removed
			mushroom[i][3] = 0;
			break;
		}
	}
